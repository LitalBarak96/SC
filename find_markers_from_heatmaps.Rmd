...
for(i in 1:ncol(lfc)){
 ...
 tmp_names<-cbind(tmp_names,i)
 numbers_genes<-as.numeric(1:nrow(log_fold_change_3))
 
  tmp_names<-cbind(tmp_names,numbers_genes)
 the_genes<-rbind(the_genes,tmp_names)

 }

}
the_genes<-as.data.frame(the_genes)
the_genes$numbers_genes<-as.numeric(the_genes$numbers_genes)
colnames(the_genes)<-c("Var","i","number")

cast_data = dcast(the_genes, number~i,value.var = "Var")

cast_data<-na.omit(cast_data)

write.csv(cast_data,"D:/OneDrive - Bar Ilan University/Lital/weekly presentation/3.9.23/npf_Each_cell.csv",na="")



#TEST<-gtools::smartbind(the_genes[[1]],the_genes[[2]],the_genes[[3]])


# for (i in 1:length(the_genes)) {
#     names(the_genes[[i]]) <-  paste0("Freq", i)
# }
# 
# 
# TTT<-Reduce(function(...) merge(..., all = T), the_genes)
...
write.csv(cast_data,"D:/OneDrive - Bar Ilan University/Lital/weekly presentation/3.9.23/npfr_Each_cell.csv",na="")
tmp_genes<-unique(the_genes)
tmp_genes<-as.data.frame(tmp_genes)
