...
# 
  ...
  if(j == 2){
    ...
    #removed
    #       if(SERUAT_HEAD@meta.data$s_Tbh[i] =f= TRUE){
...
library(pheatmap)
all<-read.csv("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/16.7.23/mat_avg_elav_wo_tbh.csv",check.names=FALSE,row.names = 1)
# write.csv(all,"D:/OneDrive - Bar Ilan University/Lital/weekly presentation/16.7.23/mat_avg_elav_wo_tbh.csv")
sig_genes<-read.csv("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/16.7.23/sig_elav_gene.csv")
...
sc_heatmap<-pheatmap(mat, scale = "row",cluster_rows  = T,cluster_cols = TRUE,show_rownames = T,fontsize_row = 5, fontsize_col = 20,cutree_cols =8,color=colorRampPalette(c("navy", "white", "red"))(50))
...
mat<-bulk_data[row.names(bulk_data) %in% genes, ]



  
bulk_heatmap<-pheatmap(mat, scale = "row",cluster_rows  = T,cluster_cols = TRUE,show_rownames = T,fontsize_row = 5, fontsize_col = 5,color=colorRampPalette(c("navy", "white", "red"))(50))
...
bulk_heatmap<-pheatmap(mat, scale = "row",cluster_rows  = T,cluster_cols = TRUE,show_rownames = T,fontsize_row = 5, fontsize_col = 20,color=colorRampPalette(c("navy", "white", "red"))(50))
```

```{r}
list_of_rowenames_genes<-rownames(mat[bulk_heatmap$tree_row[["order"]],])
normlized_bulk<-scale_mat(bulk,"row")


new_order<-normlized_bulk[(list_of_rowenames_genes), ]


plot(bulk_heatmap$tree_row)

abline(h=3, col="red", lty=2, lwd=2)

getting_cluster<-sort(cutree(bulk_heatmap$tree_row, h=3))



getting_cluster<-as.data.frame(getting_cluster)
getting_cluster<-cbind(getting_cluster,rownames(getting_cluster))

clustered_genes<-getting_cluster[(list_of_rowenames_genes), ]

clustered_genes<-as.data.frame(clustered_genes)
rownames(clustered_genes)<-list_of_rowenames_genes


splited_clustred<-split(clustered_genes,clustered_genes$getting_cluster)


```

```{r}
current_cluster_genes<-splited_clustred$`33`$`rownames(getting_cluster)`


mat<-bulk_data[row.names(bulk_data) %in% current_cluster_genes, ]
...
```{r}
cross_genes<-read.csv("C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/14.8.23/
                      120_cross.csv",header = FALSE)
cross_genes<-c(cross_genes$V1)
...
mat<-sc_data[row.names(sc_data) %in% cross_genes, ]
  ...
  plotname<-paste0("C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/14.8.23/120_crossed_sc.tiff")
 tiff(file=plotname,units="in",height=11,width=11,res=600)
...
```{r}
#to get only the cells that are npf npfr positive
sub_Seted<-readRDS("D:/seq_data/FCA/17_7_reclustered_head_elav_genes.rds")
Idents(object = sub_Seted) <- sub_Seted@meta.data$names
...
mat<-mat[apply(mat, 1, function(x) !all(x==0)),]
  ...
  plotname<-paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/27.8.23/npf_all_40_cells_gene_clusters_No_scale.tiff")
 tiff(file=plotname,units="in",height=11,width=11,res=600)
...
```{r}
npfr_de_genes<-read.csv("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/3.9.23/npfr_de_genes.csv")$X


npfr_de_genes<-c(npfr_de_genes,"Dh44")
npfr_de_genes<-c(npfr_de_genes,"NPF")
npfr_de_genes<-c(npfr_de_genes,"Tk")
npfr_de_genes<-c(npfr_de_genes,"Tk")

npfr_de_genes<-c(npfr_de_genes,"Ilp2")
npfr_de_genes<- c(npfr_de_genes,"Ilp5")
...
#rownames_sc<-as.data.frame(rownames_sc)


mat<-sc_data[row.names(sc_data) %in% npfr_de_genes, ]

  plotname<-paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/3.9.23/npfr_de_to_elav_genes_heatmap.jpeg")
...
# 
# 
# orde<-c("NPF", "Tdc2","ple","NPFR","elav","OK107","fru","Crz")
# data <- subset(mat, select=c("NPF", "Tdc2","ple","NPFR","elav","OK107","fru","Crz"))
# 
# data<-as.matrix(data)
# 
...
# colden_reordered <- reorder(colden, c("NPF", "Tdc2","ple","NPFR","elav","OK107","fru","Crz"))
