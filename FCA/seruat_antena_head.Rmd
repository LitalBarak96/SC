...
library(ggplot2)
library(dplyr)
loom_file <- connect(filename = "D:/seq_data/FCA/r_fca_biohub_head_10x.loom", mode = "r+", skip.validate=TRUE)
matrix_data <- as.data.frame(loom_file[["matrix"]][,])
  ...
  SERUAT_antena_N <- CreateSeuratObject(counts = t(matrix_data), project = "head", min.cells = 0, min.features=0)
...
par( mar= c(1,15,15,1) )
head_antena(current_gene,precentage,xlimit)
par(new=TRUE)
head_antena(current_gene,precentage,1)
