...
SERUAT_Antena <-readRDS("D:/seq_data/FCA/seuruat_antena6_12.rds")
   ...
   umap_antena<- DimPlot(SERUAT_Antena, reduction = 'umap', group.by = 'RNA_snn_res.0.8', label = TRUE,raster=FALSE)
   tiff("umao_antena.tiff", units="in", width=7, height=5, res=300)
umap_antena
dev.off()
...
obp69a<-FeaturePlot(SERUAT_Antena, "Obp69a",raster=FALSE)


   tiff("obp69a.tiff", units="in", width=7, height=5, res=300)
obp69a
dev.off()
...
plot_sex<-DimPlot(SERUAT_Antena, reduction = 'umap', group.by = 'Sex',raster=FALSE)+guides(color = guide_legend(override.aes = list(size=1), ncol=2) )+theme(legend.title = element_text(size = 5), 
               legend.text = element_text(size = 7))

tiff("sex_antena.tiff", units="in", width=7, height=5, res=300)
plot_sex
dev.off()

plots_splits_sex<-DimPlot(SERUAT_Antena, reduction = 'umap', split.by = 'Sex',raster=FALSE)+guides(color = guide_legend(override.aes = list(size=1), ncol=2) )+theme(legend.title = element_text(size = 5), 
               legend.text = element_text(size = 7))
...
obp69_sex<-FeaturePlot(object = SERUAT_Antena, "Obp69a", ncol=3,raster=FALSE,pt.size=0.5,combine=TRUE,split.by = "Sex")

tiff("sex_obps69.tiff", units="in", width=14, height=6, res=300)
obp69_sex
dev.off()
...
obps<-FeaturePlot(object = SERUAT_Antena, obps_gene[29:37], ncol=3,raster=FALSE,pt.size=0.5,combine=TRUE)


tiff("obps.tiff", units="in", width=14, height=9, res=300)
obps
dev.off()
...
#example of normaliztion and scaling

```{r}


SERUAT_Antena_new <-readRDS("D:/seq_data/FCA/seuruat_antena6_12.rds")

original_count<-SERUAT_Antena_new@assays$RNA@counts[1:10,1:10]

after_normelize<-SERUAT_Antena_new@assays[["RNA"]]@data[1:10,1:10]

SERUAT_Antena@meta.data[["nCount_RNA"]]
