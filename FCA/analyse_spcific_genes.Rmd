...
```{r}
all_gene_csv<-as.data.frame(read.csv("C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/2.7.23/all_genes.csv"))
...
# 
# saveRDS(glu,"D:/seq_data/FCA/elav_genes_reclustred_allgenes.rds")
...
#my_plot_order_names<-c(my_plot_order_names,the_last_to_plot_names)
   ...
   plotname<-paste0("C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/2.7.23/recclsuted_per_genes_croess_elav/",trimws(list_of_gene_names[gene]),".tiff")
 ...
 if(nrow(t[[list_of_gene_names[gene]]]@meta.data) > 1){
   ...
   if(nrow(t[[list_of_gene_names[gene]]]@meta.data) > 1 && nrow(t[[list_of_gene_names[gene]]]@meta.data) < 10){
        my_plot<-DimPlot(object = t[[list_of_gene_names[gene]]],cols = my_cols[list_of_gene_names[gene]],pt.size = 3 )+guides(color = guide_legend(override.aes = list(size=11,face="bold"), ncol=1) )+ggtitle(list_of_gene_names[gene])+ theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"))

plot(my_plot)
#plot(t,xaxt = "n")
dev.off()
   }else{
             my_plot<-DimPlot(object = t[[list_of_gene_names[gene]]],cols = my_cols[list_of_gene_names[gene]],pt.size = 1 )+guides(color = guide_legend(override.aes = list(size=11,face="bold"), ncol=1) )+ggtitle(list_of_gene_names[gene])+ theme(plot.title = element_text(hjust = 0.5,size = 20, face = "bold"))
plot(my_plot)
#plot(t,xaxt = "n")
dev.off()
   }
