...
plotname<-"C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/12.03.23/new_colors_1.tiff"
...
DimPlot(sub_Seted,
        cols = cols, label=TRUE ,group.by = "names",label.size
=3.5,pt.size = 1.5)
#plots[[3]] + NoLegend()  # Get just the co-expression plot, built-in legend is meaningless for this plot
#plots[[4]] # Get just the key
...
all.markers <- FindAllMarkers(object = SERUAT_HEAD,logfc.threshold = 1)
...
# do all clustring after removing what is without exprssion
```{r}
...
plotname<-"C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/19.3.23/rcluster_genes_after_removing_wo_names.tiff"
...
TEST<-subset(sub_Seted,percent.NPF > 0.3 &percent.fru>0.57)
TEST[[]]
...
DoHeatmap(object = TEST, genes.use = top20$gene, slim.col.label = TRUE, remove.key = TRUE)


p<-DoHeatmap(object = TEST)

DoHeatmap(object = TEST,features ="NPF")

test<-sub_Seted@assays$RNA@data

GetAssayData(object = TEST, slot = "counts")
```
