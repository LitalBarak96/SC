...
names(my_cols)<-list_of_gene_names
 ...
 plotname<- paste0("C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/final_outline/last_analysis/Crz.tiff")
...
all_gene_table<-read.table(file = "D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/best_gene_summary_fb_2023_05.tsv", sep = '\t', header = FALSE,fill = TRUE)

colnames(all_gene_table)<-c("Fb_name","gene","database","description")


path_for_sig<-"D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/final_outline/last_analysis/sig_Tdc2"


sig_genes<-read.csv(paste0(path_for_sig,".csv"))
library(dplyr)

#it write more things than he should 
# Using dplyr - left join multiple columns
df2 <- sig_genes %>% left_join( all_gene_table, 
            by=c('gene'='gene'))
df2


write.csv(df2,paste0(path_for_sig,"_description.csv"))
