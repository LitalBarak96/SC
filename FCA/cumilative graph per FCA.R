...
head_FCA_sum<-function(gene_name,y_value,bottom_x_value,top_x_value,FCA,NUMBER_OF_FCA){
  ...
  gene_sum_head <- hist(gene_head$head, breaks=seq(0,10,l=1000),plot = FALSE)                       # Store histogram info
  ...
  t<-plot(gene_sum_head,col=alpha('red',0.5), main=main_title, xlab='% expression',ylab='number of cells',ylim =c(0,new_yval),xlim =rev(c(bottom_x_value, top_x_value)),cex.axis=2.2,cex.lab=1.5, cex.main=2)
  dev.off()
...
for(i in 1:length(new)){
  ...
  y_Vale_max_origin<-round((13*ncol(new[[i]]))/100000) # to find how many sample suppose to be
  #y_Vale_max<-y_Vale_max_origin + 0.5 *(y_Vale_max_origin)
  head_FCA_sum("NPFR",y_Vale_max_origin,0,0.5,new[[i]],names_of_fca[i])
