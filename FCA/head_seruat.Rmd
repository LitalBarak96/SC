...
matrix_data <- as.data.frame(loom_file[["matrix"]][,])
  ...
  saveRDS(SERUAT_HEAD, "seuruat_head.rds")
...
```{r}
SERUAT_HEAD <- RunTSNE(SERUAT_HEAD, dims.use = 1:20, do.fast = T)
SERUAT_HEAD <- FindClusters(SERUAT_HEAD ,pc.use = 1:20, resolution = seq(0,2,0.4), save.SNN = T, do.sparse = T)
Idents(SERUAT_HEAD) <- "RNA_snn_res.0.4"

#check what kind of resulution is the best
DimPlot(SERUAT_HEAD, group.by = "RNA_snn_res.0.4", label = TRUE,raster=FALSE)
...
```{r}
SERUAT_HEAD <- RunUMAP(SERUAT_HEAD, dims = 1:15)
