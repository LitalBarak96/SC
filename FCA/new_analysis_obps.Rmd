...
```{r}
 SERUAT_Antena<- readRDS("D:/Lital/16_10_antena_obp_m_and_f_reclustered.rds")
...
```{r}
 ...
 SERUAT_Antena<- readRDS("D:/seq_data/FCA/16_10_antena_obp_m_and_f_reclustered.rds")

 t<-SplitObject(SERUAT_Antena, split.by = "Sex")
male_marker<-(table(t[["Male"]]@meta.data$names)/nrow(t[["Male"]]@meta.data))*100

female_marker<-(table(t[["Female"]]@meta.data$names)/nrow(t[["Female"]]@meta.data))*100


n<-names(table(SERUAT_Antena@meta.data$names))
n[1]<-"others"



  plotname<-paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/pie_all_neruons.tiff")
 tiff(file=plotname,units="in",height=15,width=15,res=600)

pie(table(SERUAT_Antena@meta.data$names),labels = n, col=rainbow(length(n)),
   main="Pie Chart of neruons all",cex=2)
...
```{r}
# SERUAT_HEAD <-readRDS("D:/seq_data/FCA/16_10_antena_obp_m_and_f_reclustered.rds")
# 
...
View(SERUAT_Antena@meta.data)
 ...
 plotname<- paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/3_male_female_",gene_name,".tiff")
...
tiff(file=plotname,units="in",height=15,width=15,res=600)
  ...
  plot(gene_sum_head,col=alpha('red',0.5),xlim=rev(c(3, x_value)),ylim =c(0,y_value),main=main_title,ylab = "sum of sampels") 
  plot(gene_sum_antena,col=alpha('green',0.5),xlim=rev(c(3, x_value)),ylim =c(0,y_value),add = TRUE)
dev.off()




 plotname<- paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/3_male_",gene_name,".tiff")
...
tiff(file=plotname,units="in",height=15,width=15,res=600)
  ...
  plot(gene_sum_antena,col=alpha('green',0.5),xlim=rev(c(3, x_value)),ylim =c(0,y_value),main=main_title,ylab = "sum of sampels") 
dev.off()

 plotname<- paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/3_female_",gene_name,".tiff")
...
names(my_cols)<-list_of_gene_names
 ...
 plotname<- paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/all_clusters.tiff")
 tiff(file=plotname,units="in",height=20,width=20,res=600)
...
```{r}
Idents(object = SERUAT_HEAD) <- SERUAT_HEAD@meta.data$Sex

t<-SplitObject(SERUAT_HEAD, split.by = "seurat_clusters")

# t_20<- t[["20"]]
# 
# Idents(object = t_20) <- t_20@meta.data$Sex

markers_clustres_20 <- FindMarkers(object = t[["20"]], ident.1 ="Female",only.pos = TRUE,logfc.threshold = 1)

markers_clustres_20<-subset(markers_clustres_20,p_val_adj<0.05)

write.csv(markers_clustres_20,"C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/cluster_num_20_male_vs_feamle.csv")


markers_clustres_21 <- FindMarkers(object = t[["21"]], ident.1 ="Female",only.pos = TRUE,logfc.threshold = 1)


markers_clustres_21<-subset(markers_clustres_21,p_val_adj<0.05)

write.csv(markers_clustres_21,"C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/cluster_num_21_male_vs_feamle.csv")


markers_clustres_23 <- FindMarkers(object =  t[["23"]], ident.1 ="Female",only.pos = TRUE,logfc.threshold = 1)



markers_clustres_23<-subset(markers_clustres_21,p_val_adj<0.05)

write.csv(markers_clustres_23,"C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/cluster_num_23_male_vs_feamle.csv")

library(purrr)

#     l <- list( rownames(markers_clustres_20),rownames(markers_clustres_21),rownames(markers_clustres_23))
# inter<-combn(length(l), 2, FUN=function(i)intersect(l[[i[1]]], l[[i[2]]]),
#       simplify=FALSE)


#intersect(intersect(a,b),c)
inter<-intersect(intersect( rownames(markers_clustres_20), rownames(markers_clustres_21)),rownames(markers_clustres_23) )


write.csv(inter,"D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/common_markers_20_21_23.csv")
