...
View(SERUAT_Antena@meta.data)
  ...
  gene_sum_head <- hist(gene_head$head,breaks=seq(0,20,l=20000))                     # Store histogram info
  gene_sum_head$counts <- revcumsum((gene_sum_head$counts))    # Change histogram counts
  
  gene_sum_antena <-hist(gene_antena$antena,breaks=seq(0,20,l=20000)) 
  gene_sum_antena$counts <- revcumsum((gene_sum_antena$counts))    # Change histogram counts

  
 plotname<- paste0("D:/OneDrive - Bar Ilan University/Lital/weekly presentation/thesis/obp_analysis/only_m_f/male_female_obp69a.tiff")
