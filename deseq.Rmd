...
res <- results(dds, name = "neuron_Tdc2_vs_Npfr")
...
DESeq2::plotPCA(vsd, intgroup="neuron") + geom_label(aes(label=name)) + labs(title = "PCA according to group - Before removing batch effect")
...
```{r}
mat <- assay(vsd)
mat <- limma::removeBatchEffect(mat, vsd$neuron)# remove according to experiment
assay(vsd) <- mat
```
...
DESeq2::plotPCA(vsd, intgroup="neuron") + geom_label(aes(label=name)) + labs(title = "PCA according to group - After removing batch effect")
