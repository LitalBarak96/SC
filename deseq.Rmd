...
coldata <- read.csv("D:/seq_data/sex_condition/meta.csv", row.names=1)
...
coldata$neruon<-factor(coldata$neruon)
...
colnames(sex_cell)<-gsub("_counts","",colnames(sex_cell))

#coldata$condition <- factor(coldata$condition)
#coldata$repeat. <- factor(coldata$repeat.)

all(rownames(sex_cell) %in% colnames(sex_cell))
all(rownames(coldata) == colnames(sex_cell))

dds <- DESeqDataSetFromMatrix(countData = round(sex_cell),
                              colData = coldata,
                              design= ~neruon+condition)
...
res <- results(dds, name = "condition_Virgin_vs_Mated")
...
DESeq2::plotPCA(vsd, intgroup="neruon") + geom_label(aes(label=name)) + labs(title = "PCA according to group - Before removing batch effect")
...
DESeq2::plotPCA(vsd, intgroup="neruon") + geom_label(aes(label=name)) + labs(title = "PCA according to group - After removing batch effect")
